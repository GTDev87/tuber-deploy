...
module.exports = {
  genericBuildAndCreate: function (location, dockerFileLocation, ssh_key, privateKeyLocation, jsonMappingStringOrCallback, possibleCallback) {
    var callback = possibleCallback || jsonMappingStringOrCallback;
    var jsonDataString = possibleCallback && jsonMappingStringOrCallback;
    ...
    pem.createCertificate({days:1, selfSigned:true, serviceKey: ((privateKeyLocation && fs.readFileSync(privateKeyLocation)) || null) }, function(err, keys){
      ...
      var dataFilenameArgs = jsonDataString ? createQuickDeployArgsAndCreateFileForTransfer("data.json", newFolder, JSON.stringify(jsonDataString), "DATA_FILENAME") : "";
      ...
      function handleMacaroonFound(){
        ...
        if (callback) {return callback(JSON.parse(clientMacaroon));}
      }
